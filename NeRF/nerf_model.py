...
def get_rays(H, W, focal, pose):
    ...
    u_normalized = (u - W * 0.5) / focal
    v_normalized = -(v - H * 0.5) / focal
    z_component = -torch.ones_like(u)
    dirs = torch.stack([u_normalized, v_normalized, z_component], dim=-1).to(device)
    ...
    rotation_part = pose[:3, :3].to(device)
    dirs_reshaped = dirs.reshape(-1, 3)
    rays_d_flat = (rotation_part @ dirs_reshaped.T).T
...
def positional_encoder(p, L_embed=6):
    ...
    for i in range(L_embed):
        ...
        sin_encoded = torch.sin(frequency * p)
        cos_encoded = torch.cos(frequency * p)

        rets.append(sin_encoded)
        rets.append(cos_encoded)
...
def render(model, rays_o, rays_d, near, far, n_samples, rand=False):
    ...
    t = t.expand(list(rays_o.shape[:-1]) + [n_samples])

    rays_o_expanded = rays_o.unsqueeze(-2)
    rays_d_expanded = rays_d.unsqueeze(-2)

    points = rays_o_expanded + rays_d_expanded * t.unsqueeze(-1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    one_e_10 = torch.tensor([1e10], device=t.device)
    dists = t[..., 1:] - t[..., :-1]
    dists = torch.cat([dists, one_e_10.expand(dists[..., :1].shape)], -1)
