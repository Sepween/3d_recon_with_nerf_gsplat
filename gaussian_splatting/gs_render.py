...
def build_covariance_2d(mean3d, cov3d, viewmatrix, fov_x, fov_y, focal_x, focal_y):
    ...
    cov2d = J[:, :2, :] @ (W @ cov3d @ W.T) @ J[:, :2, :].transpose(1, 2)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                tile_x_min, tile_y_min = w, h
                tile_x_max, tile_y_max = w + TILE_SIZE, h + TILE_SIZE

                rect_min, rect_max = rect

                rect_x_min = rect_min[:, 0]
                rect_y_min = rect_min[:, 1]
                rect_x_max = rect_max[:, 0]
                rect_y_max = rect_max[:, 1]

                in_mask = (
                    (rect_x_max >= tile_x_min) & (rect_x_min <= tile_x_max) &
                    (rect_y_max >= tile_y_min) & (rect_y_min <= tile_y_max)
                )
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
                ...
                dx_transformed = torch.einsum('bpi,pij->bpj', dx, sorted_inverse_conv)
                gauss_weight = torch.exp(-0.5 * (dx_transformed * dx).sum(-1)) # Hint: Check step 1 in the instruction pdf
                ...
                inter_value = alpha * T
                acc_alpha = inter_value.sum(dim=1) # Hint: Check Eq. (8) in the instruction pdf

                tile_color = inter_value @ sorted_color # Hint: Check Eq. (5) in the instruction pdf
                tile_depth = inter_value @ sorted_depths # Hint: Check Eq. (7) in the instruction pdf

                if self.white_bkgd:
                    remaining_alpha = 1 - acc_alpha
                    white_bkgd_color = torch.tensor([1.0, 1.0, 1.0], device=tile_color.device)
                    tile_color += remaining_alpha[..., None] * white_bkgd_color

                acc_alpha = acc_alpha.reshape(TILE_SIZE, TILE_SIZE)
                tile_color = tile_color.reshape(TILE_SIZE, TILE_SIZE, 3)
                tile_depth = tile_depth.reshape(TILE_SIZE, TILE_SIZE)
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
